...
import { isAllSolved, normalizeCtfCategory, parseKstDateTime } from "./ctf/core";
import { detectPlatform, fetchChallengesWithToken, fetchPublicChallenges, fetchPublicScoreboard, type PlatformKind } from "./ctf/platforms";
import {
  bucketForEvent as referenceBucketForEvent,
  classifyEvent as referenceClassifyEvent,
  eventContentHash,
  eventDateLabel,
  eventDedupeKey as referenceEventDedupeKey,
  eventRegion as referenceEventRegion,
  interleaveEvents as referenceInterleaveEvents,
  parseEventNotice,
  shouldIncludeEvent,
} from "./events/core";
import {
  ...
  isEditorialUrl,
  optionsFromEnv,
  searchTitleMatchesKind,
  titleYearMatchesEvent,
  type CollectionResult,
  type CollectorOptions,
} from "./events/sources";
import { translateEventsToKorean } from "./events/translation";
...
const pendingEventImports = new Map<string, { event: EventItem; userId: string; guildId: string }>();
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption((o) =>
      o
        ...
        .setRequired(false)
        .addChoices(
          { name: "정보 없음", value: "정보 없음" },
          { name: "온라인", value: "온라인" },
          { name: "오프라인", value: "오프라인" },
          { name: "온·오프라인 병행", value: "온·오프라인 병행" },
        ),
    )
...
async function registerGuild(guild: Guild) {
  if (GUILD_IDS.length && !GUILD_IDS.includes(guild.id)) return;
...
const eventSyncJobs = new Map<string, Promise<{ fetched: number; posted: number; updated: number; unchanged: number; errors: string[] }>>();
let sharedCollection: { createdAt: number; result: CollectionResult } | undefined;
let sharedCollectionJob: Promise<CollectionResult> | undefined;
const ctfMonitorTimers = new Map<string, NodeJS.Timeout>();
const ctfMonitorBusy = new Set<string>();
const ctfMonitorBackoff = new Map<string, { nextAt: number; delay: number }>();
const ctfScoreboardSnapshots = new Map<string, string>();
const ctfTeamSnapshots = new Map<string, { score: number; rank: number }>();

async function monitorCtfContest(guild: Guild, contest: CtfContest): Promise<void> {
  if (!contest.warningEnabled || !contest.publicApiReadable || !contest.sourceUrl || !contest.platform || contest.platform === "generic") return;
  ...
  try {
    const remote = await fetchPublicChallenges(contest.platform, contest.sourceUrl);
    const known = getGuildCtfProblems(guild.id).filter((problem) => problem.ctfKey === contest.key);
    ...
    for (const challenge of remote.slice(0, 100)) {
      if (knownExternal.has(challenge.externalId) || findCtfProblem(guild.id, contest.key, keyOf(challenge.name))) continue;
      const category = normalizeCtfCategory(challenge.category);
    ...
    const scoreboard = await fetchPublicScoreboard(contest.platform, contest.sourceUrl).catch((error) => {
      const message = error instanceof Error ? error.message : String(error);
      if (message === "RATE_LIMITED" || /^HTTP_5\d\d$/.test(message)) throw error;
      return [];
    });
    if (scoreboard.length) {
      ...
      if (previous && previous !== signature) {
        ...
        await feed?.send({ embeds: [new EmbedBuilder().setTitle("📊 TOP 10 scoreboard changed").setColor(0x5865f2).setDescription(body)] }).catch(() => {});
      }
      if (contest.teamName) {
        const ours = scoreboard.find((row) => keyOf(row.name) === keyOf(contest.teamName!));
...
function ensureCtfMonitor(guild: Guild) {
  if (ctfMonitorTimers.has(guild.id)) return;
  const interval = Math.max(60, Number(process.env.CTF_MONITOR_INTERVAL_SECONDS ?? 120) || 120) * 1000;
  const run = async () => {
    if (ctfMonitorBusy.has(guild.id)) return;
    ctfMonitorBusy.add(guild.id);
    try {
      for (const contest of getGuildCtfContests(guild.id)) await monitorCtfContest(guild, contest);
    } finally {
...
async function collectEventsShared(options: CollectorOptions): Promise<CollectionResult> {
  if (sharedCollection && Date.now() - sharedCollection.createdAt < 5 * 60_000) return sharedCollection.result;
  if (sharedCollectionJob) return sharedCollectionJob;
  sharedCollectionJob = collectEventItems(options);
  try {
...
function releaseUnusedMemory(): void {
  const gc = (globalThis as typeof globalThis & { gc?: () => void }).gc;
  if (gc) setTimeout(() => gc(), 500).unref();
}
...
client.once(Events.ClientReady, async (c) => {
  ...
  for (const guild of c.guilds.cache.values()) {
    ...
    if (getFeatures(guild.id).includes("ctf")) ensureCtfMonitor(guild);
  }
...
// ── 패널 렌더링 ───────────────────────────────────────────────────────
function buildSourceSelect(includeCtfManagement: boolean) {
  const options = [
  ...
  const menu = new StringSelectMenuBuilder().setCustomId("src_select").setPlaceholder("문제 출처를 고르세요").addOptions(options);
  return {
...
async function ensureCtfTextChannel(
  guild: Guild,
  ctfKey: string,
  categoryId: string,
  key: string,
  ...
  const existingId = getForumFor(guild.id, storeKey);
  if (existingId) {
  ...
  const channel = await guild.channels.create({ name, type: ChannelType.GuildText, parent: categoryId, topic });
  setForumFor(guild.id, storeKey, channel.id);
  return channel;
}

/** CTF 참가자 역할 + 비공개 카테고리 + 기본 텍스트 채널 확보 */
async function getOrCreateCtf(
  guild: Guild,
  ctfName: string,
  options: { startsAt?: number; endsAt?: number; teamName?: string } = {},
) {
  ...
  if (!category || category.type !== ChannelType.GuildCategory) {
    category = await guild.channels.create({
      ...
      permissionOverwrites: [
        ...
        { id: guild.members.me!.id, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages, PermissionFlagsBits.ManageChannels, PermissionFlagsBits.ManageThreads, PermissionFlagsBits.CreatePublicThreads] },
      ],
    });
  ...
  const oldTime = getCtfTime(guild.id, ctfKey);
  ...
  setCtfTime(guild.id, ctfKey, startsAt, endsAt);
  let contest = getCtfContest(guild.id, ctfKey);
  if (!contest) {
    contest = {
    ...
    upsertCtfContest(contest);
  } else {
    contest = updateCtfContest(guild.id, ctfKey, {
  ...
  if (created || !contest.lobbyMessageId) {
    ...
    if (message) contest = updateCtfContest(guild.id, ctfKey, { lobbyChannelId: lobby.id, lobbyMessageId: message.id })!;
  }
  return { categoryId: category.id, roleId: role.id, ctfKey, contest };
}

/** 이미지와 같은 장르 텍스트 채널. 문제는 이 채널의 공개 스레드로 생성한다. */
async function ensureGenreForum(guild: Guild, ctfKey: string, categoryId: string, _roleId: string, genre: string): Promise<TextChannel> {
  const normalized = normalizeCtfCategory(genre);
...
async function resetEventFeature(guild: Guild): Promise<{ channels: number; items: number; failed: number }> {
  ...
  const categoryIds = new Set(
    guild.channels.cache
      .filter((channel) => channel.type === ChannelType.GuildCategory && categoryNames.has(channel.name))
      .map((channel) => channel.id),
  );
  const channelIds = new Set(
    keys
      .map((key) => getForumFor(guild.id, key))
      .filter((id): id is string => Boolean(id)),
  );
  for (const channel of guild.channels.cache.values()) {
    if (channel.parentId && categoryIds.has(channel.parentId)) channelIds.add(channel.id);
  }
  for (const categoryId of categoryIds) channelIds.delete(categoryId);
  ...
  for (const key of keys) {
    ...
    if (!channelId || removedIds.has(channelId) || !guild.channels.cache.has(channelId)) removeForumFor(guild.id, key);
  }
...
function contestForChannel(guildId: string, channelId: string, parentId?: string | null): CtfContest | undefined {
  return getGuildCtfContests(guildId).find(
    (contest) => contest.categoryId === channelId || contest.categoryId === parentId,
  );
}

async function ctfCoreChannel(guild: Guild, ctfKey: string, key: string): Promise<TextChannel | null> {
  const id = getForumFor(guild.id, `ctftext:${ctfKey}:${key}`);
  if (!id) return null;
  const channel = guild.channels.cache.get(id) ?? await guild.channels.fetch(id).catch(() => null);
  return channel?.type === ChannelType.GuildText ? channel as TextChannel : null;
}

function allSolveEmbed(contest: CtfContest, solved: number, total: number): EmbedBuilder {
  return new EmbedBuilder()
...
async function refreshAllSolveStatus(guild: Guild, ctfKey: string): Promise<boolean> {
  let contest = getCtfContest(guild.id, ctfKey);
  if (!contest) return false;
  const problems = getGuildCtfProblems(guild.id).filter((problem) => problem.ctfKey === ctfKey);
  const allSolved = isAllSolved(problems);
  const changed = contest.allSolved !== allSolved;
  contest = updateCtfContest(guild.id, ctfKey, { allSolved })!;
  const channel = await ctfCoreChannel(guild, ctfKey, "solve");
  if (!channel) return allSolved;
  ...
  if (message) await message.edit({ embeds: [embed] }).catch(() => {});
    ...
    if (message) updateCtfContest(guild.id, ctfKey, { solveStatusMessageId: message.id });
  }
...
client.on(Events.InteractionCreate, async (interaction) => {
  try {
    ...
    if (interaction.isRepliable() && !interaction.replied && !interaction.deferred) {
      await interaction.reply({ content: "⚠️ 처리 중 오류가 발생했습니다.", flags: MessageFlags.Ephemeral }).catch(() => {});
    }
  }
});
...
async function handleCommand(interaction: ChatInputCommandInteraction) {
  ...
  if (feat && interaction.guildId && !getFeatures(interaction.guildId).includes(feat)) {
    return interaction.reply({ content: "이 기능은 꺼져 있어요. `/봇 기능 추가` 로 켜주세요.", flags: MessageFlags.Ephemeral });
  }
  if (name === "문제") return handleProblemCommand(interaction);
  if (name === "ctf" || name === "ctf관리") return handleCtfCommand(interaction);
...
async function handleBotCommand(interaction: ChatInputCommandInteraction) {
  if (!interaction.guild) return interaction.reply({ content: "서버 안에서만 사용할 수 있습니다.", flags: MessageFlags.Ephemeral });
  if (!isBotOwner(interaction)) return interaction.reply({ content: ownerOnlyMessage(), flags: MessageFlags.Ephemeral });
  ...
  if (sub === "추가") {
    ...
    if (off.length === 0) return interaction.reply({ content: "이미 모든 기능이 켜져 있어요.", flags: MessageFlags.Ephemeral });
  ...
  if (sub === "삭제") {
    if (enabled.length === 0) return interaction.reply({ content: "켜진 기능이 없어요.", flags: MessageFlags.Ephemeral });
    const menu = new StringSelectMenuBuilder()
...
async function handleLoggingCommand(interaction: ChatInputCommandInteraction) {
  ...
  if (interaction.commandName === "로그채널") {
    ...
    return interaction.reply({ content: `✅ 로그 채널이 <#${channel.id}> 로 설정됐어요!`, flags: MessageFlags.Ephemeral });
  }
  // 로그채널확인
  const saved = getLogChannel(interaction.guild.id);
  if (!saved) return interaction.reply({ content: "❌ 설정된 로그 채널이 없어요. `/로그채널 #채널` 로 설정하세요.", flags: MessageFlags.Ephemeral });
  return interaction.reply({ content: `📌 현재 로그 채널: <#${saved}>`, flags: MessageFlags.Ephemeral });
}
...
const DEFAULT_EVENT_FEEDS: { url: string; kind: "news" | "event" }[] = [
...
function eventFeedUrls(): { url: string; kind: "news" | "event" }[] {
  const extra = (process.env.EVENT_FEED_URLS ?? "")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean)
    .map((url) => ({ url, kind: "event" as const }));
...
function eventDedupeKey(item: EventItem): string {
  return referenceEventDedupeKey(item);
}
...
function classifyEvent(title: string, summary: string): string {
  return referenceClassifyEvent(title, summary);
}
...
function bucketForEvent(item: Pick<EventItem, "kind" | "title" | "startsAt" | "endsAt" | "publishedAt">): string {
  return referenceBucketForEvent(item);
}

async function fetchEventFeed(url: string, feedKind: "news" | "event" = "event"): Promise<EventItem[]> {
  ...
  for (const block of blocks) {
    ...
    if (feedKind === "news") {
      ...
      if (looksLikeResultNews(title, summary)) continue;
      kind = classifyEvent(title, summary);
      if (kind === "news") continue; // 행사 신호가 없으면 행사 채널에 올리지 않음
    }
...
type EventWriteResult = "created" | "updated" | "unchanged";

async function createEventPost(guild: Guild, item: EventItem, updated = false): Promise<{ threadId: string; messageId: string }> {
...
async function upsertEventItem(guild: Guild, item: EventItem): Promise<EventWriteResult> {
  ...
  const existing = getEventItem(guild.id, next.id) ?? getGuildEventItems(guild.id).find((saved) => eventDedupeKey(saved) === dedupeKey);
  const destination = eventForumKey(next);
  const contentHash = eventContentHash(next);

  if (!existing) {
    const posted = await createEventPost(guild, next);
    addEventItem({ ...next, ...posted, destination, contentHash, postedAt: Date.now() });
    return "created";
  }
  ...
  if (existing.contentHash === contentHash && oldDestination === destination && threadId) {
    ...
    if (thread?.isThread()) {
      ...
      if (thread.name !== eventPostTitle(next)) await thread.setName(eventPostTitle(next)).catch(() => {});
      updateEventItem(guild.id, existing.id, { ...existing, ...next, destination, contentHash, threadId });
      return "unchanged";
    }
  }

  if (oldDestination !== destination || !threadId) {
    if (threadId) await deleteChannelSafe(threadId);
    const posted = await createEventPost(guild, next, Boolean(existing.contentHash));
    updateEventItem(guild.id, existing.id, { ...next, ...posted, destination, contentHash, postedAt: Date.now() });
  ...
  if (thread?.isThread()) {
    ...
    if (message) {
      ...
      updateEventItem(guild.id, existing.id, {
        ...existing,
        ...next,
        destination,
        contentHash,
...
function eventRegion(item: EventItem): "kr" | "global" {
  return referenceEventRegion(item);
}

function eventEmbed(item: EventItem, updated = false): EmbedBuilder {
  ...
  if (item.location) embed.addFields({ name: "장소", value: item.location, inline: true });
...
async function syncEvents(guild: Guild): Promise<{ fetched: number; posted: number; updated: number; unchanged: number; errors: string[] }> {
...
async function synchronizeEvents(guild: Guild): Promise<{ fetched: number; posted: number; updated: number; unchanged: number; errors: string[] }> {
  const collectorOptions = optionsFromEnv();
  const collected = await collectEventsShared(collectorOptions);
  const errors = [...collected.errors];
  let saved = getGuildEventItems(guild.id);
  for (const item of saved) {
    if (item.kind === "news") continue;
    const editorial = isEditorialUrl(item.link) || Boolean(item.registrationUrl && isEditorialUrl(item.registrationUrl));
    const discoveryItem = /검색API|자동탐색|Google 뉴스/i.test(item.source);
    const validTitle = searchTitleMatchesKind((item.kind ?? "security") as "ctf" | "ai" | "conference" | "hackathon" | "security" | "news", item.title);
    const validYear = titleYearMatchesEvent(item.title, item);
    if (!editorial && validYear && (!discoveryItem || validTitle)) continue;
    const deleted = await deleteChannelSafe(item.threadId ?? item.messageId ?? "");
    if (!deleted) {
      errors.push(`오탐 게시물 삭제 실패: ${item.title}`);
      continue;
    }
    removeEventItem(guild.id, item.id);
  }
  saved = getGuildEventItems(guild.id);
  ...
  for (const item of candidates) {
    ...
    const next: EventItem = { ...item, guildId: guild.id, startsAt };
    next.kind = next.kind ?? classifyEvent(next.title, next.summary ?? "");
    next.region ??= eventRegion(next);
    next.bucket = bucketForEvent(next);
    const dedupeKey = eventDedupeKey(next);
    const tracked = savedIds.has(next.id) || savedByDedupe.has(dedupeKey);
    if (!shouldIncludeEvent(next, { lookaheadDays: collectorOptions.lookaheadDays, tracked })) continue;
  ...
  if (envBool("ENABLE_TRANSLATION", true)) all = await translateEventsToKorean(all);
  const news = all
  ...
  const ordered = referenceInterleaveEvents([...nonNews, ...news]).slice(0, 150);
...
async function handleEventCommand(interaction: ChatInputCommandInteraction) {
  ...
  if (interaction.commandName === "event_sync") {
    if (!isAdmin(interaction)) return interaction.reply({ content: "⛔ 관리자만 사용할 수 있습니다.", flags: MessageFlags.Ephemeral });
    ...
    try {
      ...
      return interaction.editReply(
        `✅ 수집 완료: ${result.fetched}개 확인 · 신규 ${result.posted} · 수정/이동 ${result.updated} · 변경 없음 ${result.unchanged}${failed}`,
      );
    } catch (e: any) {
  ...
  if (interaction.commandName === "event_add") {
    ...
    if (link && !/^https?:\/\/\S+$/i.test(link)) {
      return interaction.reply({ content: "링크는 http:// 또는 https://로 시작해야 합니다.", flags: MessageFlags.Ephemeral });
    }
    ...
    if ((deadlineText && !registrationDeadline) || (startText && !startsAt) || (endText && !endsAt)) {
      return interaction.reply({ content: "날짜 형식이 올바르지 않습니다. YYYY-MM-DD 형식으로 입력하세요.", flags: MessageFlags.Ephemeral });
    }
    ...
    return interaction.reply({ content: result === "created" ? "✅ 보안뉴스/행사를 등록했습니다." : result === "updated" ? "✅ 기존 항목을 갱신했습니다." : "이미 동일한 항목입니다.", flags: MessageFlags.Ephemeral });
  }

  if (interaction.commandName === "event_import") {
    if (!isAdmin(interaction)) return interaction.reply({ content: "⛔ 관리자만 사용할 수 있습니다.", flags: MessageFlags.Ephemeral });
    const modal = new ModalBuilder().setCustomId("eventimport").setTitle("행사 공지 가져오기").addComponents(
  ...
  if (interaction.commandName === "event_import_url") {
    ...
    try {
      ...
      const item = parseEventNotice(notice, "Direct");
      item.link ||= url;
      ...
      const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder().setCustomId(`eventconfirm:${token}`).setLabel("등록").setStyle(ButtonStyle.Success),
        new ButtonBuilder().setCustomId(`eventcancel:${token}`).setLabel("취소").setStyle(ButtonStyle.Danger),
      );
  ...
  if (interaction.commandName === "event_remove") {
    ...
    if (items.length === 0) return interaction.reply({ content: "삭제할 항목이 없습니다.", flags: MessageFlags.Ephemeral });
    const menu = new StringSelectMenuBuilder()
  ...
  if (interaction.commandName === "event_reset") {
    ...
    if (interaction.options.getBoolean("confirm") === true) {
      ...
      return interaction.editReply(
        `🧨 리셋 완료: 채널/카테고리 ${result.channels}개 삭제, 수집 기록 ${result.items}개 초기화` +
  ...
  if (interaction.commandName === "event_list_manual") {
    const items = getGuildEventItems(interaction.guild.id).filter((item) => item.manual).slice(0, 25);
    if (items.length === 0) return interaction.reply({ content: "수동 등록 행사가 없습니다.", flags: MessageFlags.Ephemeral });
    const embed = new EmbedBuilder()
      .setTitle("수동 등록 행사")
      .setColor(0x2b8a3e)
      .setDescription(
  ...
  if (items.length === 0) return interaction.reply({ content: "아직 수집된 보안뉴스/행사가 없습니다. `/event_sync`를 먼저 실행하세요.", flags: MessageFlags.Ephemeral });
  const embed = new EmbedBuilder()
    .setTitle("최근 보안뉴스 · 행사")
    .setColor(0x2b8a3e)
...
async function handleProblemCommand(interaction: ChatInputCommandInteraction) {
  const sub = interaction.options.getSubcommand();
  if (sub === "생성") return interaction.reply(buildSourceSelect(isBotOwner(interaction)));

  if (!interaction.guild) return interaction.reply({ content: "서버 안에서만 사용할 수 있습니다.", flags: MessageFlags.Ephemeral });

  if (sub === "삭제") {
    const problems = getGuildProblems(interaction.guild.id);
    if (problems.length === 0) return interaction.reply({ content: "삭제할 문제가 없습니다.", flags: MessageFlags.Ephemeral });
    const menu = new StringSelectMenuBuilder()
...
async function handleCtfCommand(interaction: ChatInputCommandInteraction) {
  ...
  const threadProblem = getCtfProblemByPost(interaction.channelId);
  const contest = (threadProblem ? getCtfContest(guildId, threadProblem.ctfKey) : undefined)
  ...
  if (["create", "createchallenge", "edit", "deletechallenge", "addpoint", "deletepoint", "warning", "추가", "대회삭제", "시간", "점수추가", "pull", "import"].includes(sub) && !isBotOwner(interaction)) {
    return interaction.reply({ content: ownerOnlyMessage(), flags: MessageFlags.Ephemeral });
  }

  if (sub === "create") {
    const name = interaction.options.getString("name", true).trim();
    const startsAt = parseKstDateTime(interaction.options.getString("start", true));
    const endsAt = parseKstDateTime(interaction.options.getString("end", true));
    const teamName = interaction.options.getString("team")?.trim() || undefined;
    if (!startsAt || !endsAt || endsAt <= startsAt) {
      return interaction.reply({ content: "일정은 한국 시간 `YYYY-MM-DD HH:mm` 형식으로 입력하고 종료 시각은 시작 이후여야 합니다.", flags: MessageFlags.Ephemeral });
    }
    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
  ...
  if (sub === "createchallenge") {
    if (!contest) return interaction.reply({ content: "이 명령은 `/ctf create`로 만든 CTF 카테고리 안에서 실행하세요.", flags: MessageFlags.Ephemeral });
    const category = normalizeCtfCategory(interaction.options.getString("category", true));
    const name = interaction.options.getString("name", true).trim();
    if (findCtfProblem(guildId, contest.key, keyOf(name))) return interaction.reply({ content: "같은 이름의 문제가 이미 있습니다.", flags: MessageFlags.Ephemeral });
    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
  ...
  if (sub === "info") {
    if (!contest) return interaction.reply({ content: "CTF 작업 공간 안에서 실행하세요.", flags: MessageFlags.Ephemeral });
    const problems = getGuildCtfProblems(guildId).filter((problem) => problem.ctfKey === contest.key);
    const solved = problems.filter((problem) => problem.solved).length;
    const embed = new EmbedBuilder().setTitle(`🚩 ${contest.name}`).setColor(contest.allSolved ? 0x3498db : 0xffffff)
      .addFields(
        { name: "일정", value: `<t:${Math.floor(contest.startsAt / 1000)}:f> ~ <t:${Math.floor(contest.endsAt / 1000)}:f>` },
        { name: "진행", value: `${solved}/${problems.length} solved`, inline: true },
        { name: "모니터", value: contest.warningEnabled ? "켜짐" : "꺼짐", inline: true },
      );
    if (contest.sourceUrl) embed.setURL(contest.sourceUrl);
  ...
  if (sub === "edit") {
    ...
    const startsAt = startInput ? parseKstDateTime(startInput) : contest.startsAt;
    const endsAt = endInput ? parseKstDateTime(endInput) : contest.endsAt;
    if (!startsAt || !endsAt || endsAt <= startsAt) return interaction.reply({ content: "일정은 KST `YYYY-MM-DD HH:mm` 형식이며 종료가 시작 이후여야 합니다.", flags: MessageFlags.Ephemeral });
    const teamName = interaction.options.getString("team")?.trim() || contest.teamName;
    const updated = updateCtfContest(guildId, contest.key, { startsAt, endsAt, teamName })!;
    setCtfTime(guildId, contest.key, startsAt, endsAt);
    ...
    return interaction.reply({ content: `✅ **${updated.name}** 정보를 수정했습니다.`, flags: MessageFlags.Ephemeral });
  }

  if (sub === "history" || sub === "profile") {
    const target = sub === "profile" ? (interaction.options.getUser("user") ?? interaction.user) : interaction.user;
    const records = getGuildCtfProblems(guildId).flatMap((problem) => {
    ...
    return interaction.reply({ embeds: [new EmbedBuilder().setTitle(`${target.username} · CTF profile`).setColor(0x5865f2)
      .setDescription(lines.length ? lines.join("\n") : "기록이 없습니다.").setFooter({ text: `총 ${total}점 · ${records.length}개 기록` })] });
  }

  if (sub === "defaultsettings") {
    return interaction.reply({ content: "기본값: 한국 시간(KST) · Solve 1점 · Contribute 0.5점 · 카테고리 소문자 · 감시 120초(최소 60초) · 오류 시 최대 15분 백오프", flags: MessageFlags.Ephemeral });
  }
  ...
  if (sub === "warning") {
    ...
    if (enabled && (!contest.publicApiReadable || !contest.sourceUrl || !contest.platform || contest.platform === "generic")) {
      return interaction.reply({ content: "먼저 `/ctf pull`로 CTFd/rCTF 문제를 가져와 플랫폼 주소를 연결하세요. 지원되지 않는 사이트는 수동 등록만 사용합니다.", flags: MessageFlags.Ephemeral });
    }
    updateCtfContest(guildId, contest.key, { warningEnabled: enabled });
    ensureCtfMonitor(interaction.guild);
    return interaction.reply({ content: enabled ? "🔔 저부하 새 문제 감시를 켰습니다. 읽기 전용으로 최소 120초 간격을 사용합니다." : "🔕 새 문제 감시를 껐습니다.", flags: MessageFlags.Ephemeral });
  }

  if (sub === "추가") {
    ctfDrafts.set(interaction.user.id, {});
    return interaction.reply({ ...buildCtfPanel({}), flags: MessageFlags.Ephemeral });
  }

  if (sub === "solve") {
    const p = getCtfProblemByPost(interaction.channelId);
    if (!p) return interaction.reply({ content: "이 명령은 **CTF 문제 게시글(스레드) 안**에서 사용하세요.", flags: MessageFlags.Ephemeral });
    if (p.solved) return interaction.reply({ content: "이미 풀린 문제예요. (처음 푼 사람만 인정)", flags: MessageFlags.Ephemeral });
  ...
  if (sub === "수정") {
    ...
    if (problems.length === 0) return interaction.reply({ content: "수정할 CTF 문제가 없습니다.", flags: MessageFlags.Ephemeral });
    const menu = new StringSelectMenuBuilder()
  ...
  if (sub === "삭제" || sub === "deletechallenge") {
    ...
    if (sub === "deletechallenge" && contest) problems = problems.filter((problem) => problem.ctfKey === contest.key);
    if (problems.length === 0) return interaction.reply({ content: "삭제할 CTF 문제가 없습니다.", flags: MessageFlags.Ephemeral });
    const menu = new StringSelectMenuBuilder()
  ...
  if (sub === "대회삭제") {
    ...
    for (const existing of getGuildCtfContests(guildId)) seen.set(existing.key, { name: existing.name, count: 0 });
    for (const p of problems) {
      const e = seen.get(p.ctfKey) ?? { name: p.ctfName, count: 0 };
      e.count++;
      seen.set(p.ctfKey, e);
    }
    if (seen.size === 0) return interaction.reply({ content: "삭제할 CTF가 없습니다.", flags: MessageFlags.Ephemeral });
    const menu = new StringSelectMenuBuilder()
  ...
  if (sub === "시간") {
    ...
    if (!dur) return interaction.reply({ content: "기간을 인식하지 못했어요. 예: `24h`, `2d`, `1d12h`, `90m`", flags: MessageFlags.Ephemeral });
  ...
  if (sub === "점수추가" || sub === "addpoint") {
    ...
    let problems = getGuildCtfProblems(guildId);
    if (sub === "addpoint" && contest) problems = problems.filter((problem) => problem.ctfKey === contest.key);
    if (problems.length === 0) return interaction.reply({ content: "CTF 문제가 없습니다.", flags: MessageFlags.Ephemeral });
    const menu = new StringSelectMenuBuilder()
  ...
  if (sub === "deletepoint") {
    const target = interaction.options.getUser("user", true);
    const problems = getGuildCtfProblems(guildId).filter((problem) => problem.solves[target.id] != null && (!contest || problem.ctfKey === contest.key));
    if (!problems.length) return interaction.reply({ content: "이 유저의 삭제 가능한 기록이 없습니다.", flags: MessageFlags.Ephemeral });
    const menu = new StringSelectMenuBuilder().setCustomId(`ctfdeletepoint:${target.id}`).setPlaceholder("삭제할 기록")
    ...
    return interaction.reply({ content: `<@${target.id}>의 삭제할 기록을 선택하세요.`, components: [new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(menu)], flags: MessageFlags.Ephemeral });
  }

  if (sub === "pull") {
    if (!isAdmin(interaction)) return interaction.reply({ content: "⛔ 관리자만 사용할 수 있습니다.", flags: MessageFlags.Ephemeral });
    const modal = new ModalBuilder().setCustomId("ctfpull").setTitle("CTF 문제 읽어오기").addComponents(
      new ActionRowBuilder<TextInputBuilder>().addComponents(
      ...
      new ActionRowBuilder<TextInputBuilder>().addComponents(
        new TextInputBuilder().setCustomId("ctfname").setLabel("이 CTF 이름").setStyle(TextInputStyle.Short).setRequired(true).setMaxLength(80).setValue(contest?.name ?? "CTF"),
      ),
      new ActionRowBuilder<TextInputBuilder>().addComponents(
      ...
      new ActionRowBuilder<TextInputBuilder>().addComponents(
        new TextInputBuilder().setCustomId("username").setLabel("아이디 (공개 API면 비워두기)").setStyle(TextInputStyle.Short).setRequired(false),
      ),
      new ActionRowBuilder<TextInputBuilder>().addComponents(
        new TextInputBuilder().setCustomId("password").setLabel("비밀번호 (저장하지 않음)").setStyle(TextInputStyle.Short).setRequired(false),
      ),
    );
    return interaction.showModal(modal);
  }
...
async function handleButton(interaction: ButtonInteraction) {
  ...
  if (id.startsWith("eventconfirm:") || id.startsWith("eventcancel:")) {
    ...
    if (!pending || pending.userId !== interaction.user.id || pending.guildId !== interaction.guildId) {
      return interaction.reply({ content: "만료되었거나 본인의 등록 요청이 아닙니다.", flags: MessageFlags.Ephemeral });
    }
  ...
  if (id.startsWith("flag:")) {
    ...
    if (!getProblem(pid)) return interaction.reply({ content: "이미 삭제된 문제입니다.", flags: MessageFlags.Ephemeral });
  ...
  if (id.startsWith("ctftry:")) {
    ...
    if (!p) return interaction.reply({ content: "이미 삭제된 문제입니다.", flags: MessageFlags.Ephemeral });
  ...
  if (id.startsWith("ctfjoin:")) {
    ...
    if (!roleId) return interaction.reply({ content: "대회를 찾을 수 없습니다.", flags: MessageFlags.Ephemeral });
    ...
    return interaction.reply({ content: "🙌 참가 완료! 이제 이 대회의 문제 게시판이 보입니다.", flags: MessageFlags.Ephemeral });
  }
...
async function confirmCtfSolve(interaction: ButtonInteraction) {
  ...
  if (interaction.guild) {
    const thread = await client.channels.fetch(p.postId).catch(() => null);
    if (thread?.isThread()) await thread.setName(`✅｜${p.name}`.slice(0, 100)).catch(() => {});
    ...
    await solveCh?.send({ embeds: [new EmbedBuilder()
      .setTitle(`✅ ${p.name}`)
...
async function handleModal(interaction: ModalSubmitInteraction) {
  ...
  if (id === "eventimport") {
    ...
    try {
      let notice = raw;
      if (/^https?:\/\/\S+$/i.test(raw)) notice = await fetchNoticeText(raw);
      const item = parseEventNotice(notice, "Direct");
      if (!item.link && /^https?:\/\//i.test(raw)) item.link = raw;
  ...
  if (id.startsWith("fm:")) {
    ...
    if (!problem) return interaction.reply({ content: "이미 삭제된 문제입니다.", flags: MessageFlags.Ephemeral });
    const value = interaction.fields.getTextInputValue("value").trim();
    if (value !== problem.flag.trim()) return interaction.reply({ content: "❌ 플래그가 틀렸습니다.", flags: MessageFlags.Ephemeral });
  ...
  if (id.startsWith("ctfedit:")) {
    ...
    const p = getCtfProblem(pid);
    if (!p) return interaction.reply({ content: "이미 삭제된 문제입니다.", flags: MessageFlags.Ephemeral });
    if (!canManage(interaction, p.authorId)) return interaction.reply({ content: "⛔ 출제자/관리자만 수정할 수 있습니다.", flags: MessageFlags.Ephemeral });
    ...
    return interaction.reply({ content: `✏️ **${newName}** (${newGenre}) 로 수정했습니다.`, flags: MessageFlags.Ephemeral });
  }
}
...
async function handleSelect(interaction: StringSelectMenuInteraction) {
  ...
  if (cid === "feat_add" || cid === "feat_del") {
    ...
    if (cid === "feat_add" && selected.includes("ctf")) ensureCtfMonitor(interaction.guild);
  ...
  if (cid === "del_select") {
    ...
    if (!canManage(interaction, problem.authorId)) return interaction.reply({ content: "⛔ 출제자/관리자만 삭제할 수 있습니다.", flags: MessageFlags.Ephemeral });
  ...
  if (cid === "ctfdel_select") {
    ...
    if (!p) return interaction.update({ content: "이미 삭제된 문제입니다.", components: [] });
    if (!canManage(interaction, p.authorId)) return interaction.reply({ content: "⛔ 출제자/관리자만 삭제할 수 있습니다.", flags: MessageFlags.Ephemeral });
    await interaction.update({ content: `🗑️ '${p.name}' 삭제 중...`, components: [] });
    await deleteChannelSafe(p.postId);
    removeCtfProblem(p.id);
    if (interaction.guild) await refreshAllSolveStatus(interaction.guild, p.ctfKey);
  ...
  if (cid === "ctfwipe_select") {
    ...
    const contest = getCtfContest(guildId, key);
    if (!contest && probs.length === 0) return interaction.update({ content: "이미 삭제된 CTF입니다.", components: [] });
    ...
    const catId = contest?.categoryId ?? getForumFor(guildId, `ctfcat:${key}`);
    if (catId) {
      await interaction.guild.channels.fetch();
      for (const channel of interaction.guild.channels.cache.values()) {
        if (channel.parentId === catId) await deleteChannelSafe(channel.id);
      }
    }
    if (catId) {
    ...
    for (const prefix of [`ctf:${key}:`, `ctftext:${key}:`]) {
      for (const storeKey of getForumKeysFor(guildId, prefix)) removeForumFor(guildId, storeKey);
    }
    // 역할 삭제
  ...
  if (cid.startsWith("ctfadd:")) {
    ...
    const p = getCtfProblem(interaction.values[0]);
    if (!p) return interaction.update({ content: "이미 삭제된 문제입니다.", components: [] });
    if (amount === 0.5 && !p.solved) return interaction.update({ content: "미해결 문제에는 Contribute만 단독으로 추가할 수 없습니다. Solve 기록을 먼저 추가하세요.", components: [] });
    setCtfSolve(p.id, targetId, amount);
    const thread = await client.channels.fetch(p.postId).catch(() => null);
    if (p.solved && thread?.isThread()) await thread.setName(`✅｜${p.name}`.slice(0, 100)).catch(() => {});
  ...
  if (cid.startsWith("ctfdeletepoint:")) {
    if (!interaction.guild || !isBotOwner(interaction)) return interaction.reply({ content: ownerOnlyMessage(), flags: MessageFlags.Ephemeral });
    const targetId = cid.slice("ctfdeletepoint:".length);
    const problem = getCtfProblem(interaction.values[0]);
    if (!problem || !deleteCtfSolve(problem.id, targetId)) return interaction.update({ content: "이미 삭제된 기록입니다.", components: [] });
    if (!problem.solved) {
      const thread = await client.channels.fetch(problem.postId).catch(() => null);
      if (thread?.isThread()) await thread.setName(problem.name.slice(0, 100)).catch(() => {});
    }
...
async function finalize(interaction: ButtonInteraction) {
  ...
  if (!state?.name || !state.flag || !state.tier || !state.genre) {
    return interaction.reply({ content: "이름·정답·장르·티어를 모두 입력해야 합니다.", flags: MessageFlags.Ephemeral });
  }
  if (!interaction.guild) return interaction.reply({ content: "서버 안에서만 사용할 수 있습니다.", flags: MessageFlags.Ephemeral });
  await interaction.update({ content: "⏳ 문제를 생성하는 중...", embeds: [], components: [] });

  const guild = interaction.guild;
  const genre = normalizeCtfCategory(state.genre);
...
async function finalizeCtf(interaction: ButtonInteraction) {
  ...
  if (!state?.ctfName || !state.genre || !state.name) {
    return interaction.reply({ content: "CTF 이름·장르·문제 이름을 모두 입력해야 합니다.", flags: MessageFlags.Ephemeral });
  }
  if (!interaction.guild) return interaction.reply({ content: "서버 안에서만 사용할 수 있습니다.", flags: MessageFlags.Ephemeral });
  if (!isBotOwner(interaction)) return interaction.reply({ content: ownerOnlyMessage(), flags: MessageFlags.Ephemeral });
  ...
  if (findCtfProblem(guild.id, ctfKey, keyOf(name))) {
    return interaction.reply({ content: `이미 **${ctfName}** 에 같은 이름의 문제가 있습니다.`, flags: MessageFlags.Ephemeral });
  }
...
async function handleCtfImportModal(interaction: ModalSubmitInteraction) {
  ...
  const forumCache = new Map<string, TextChannel>();
  let created = 0;
  let skipped = 0;
...
async function handleCtfPullModal(interaction: ModalSubmitInteraction) {
  ...
  let list: { externalId: string; name: string; category: string }[];
  let platform: PlatformKind = "generic";
  ...
  try {
    ...
    if (platform !== "generic") {
      ...
      const result = await fetchChallengesWithToken(url, username);
  ...
  if (list.length === 0) return interaction.editReply("⚠️ 연결은 됐지만 현재 공개된 문제가 없습니다.");

  const guild = interaction.guild;
  const dur = parseDuration(interaction.fields.getTextInputValue("duration") ?? "");
  const current = getCtfContest(guild.id, keyOf(ctfName));
  ...
  updateCtfContest(guild.id, ctfKey, { platform, sourceUrl: url, publicApiReadable: publicReadable });
  const forumCache = new Map<string, TextChannel>();
  let created = 0;
  let skipped = 0;
  for (const c of list.slice(0, 50)) {
    ...
    if (!name) continue;
    const genre = normalizeCtfCategory(c.category || "misc");
    if (findCtfProblem(guild.id, ctfKey, keyOf(name))) {
      skipped++;
      continue;
    }
