...
const adminSubs = new Set(["create", "createchallenge", "edit", "deletechallenge", "addpoint", "deletepoint", "pull", "warning", "defaultsettings"]);
...
async function workspace(guild, name, startsAt, endsAt, teamName) {
    ...
    if (!contest.lobbyMessageId) {
        const lobby = await guild.channels.create({ name: `dawn-${key}`.slice(0, 95), type: discord_js_1.ChannelType.GuildText, permissionOverwrites: [{ id: guild.roles.everyone.id, deny: [discord_js_1.PermissionFlagsBits.SendMessages] }] });
        const m = await lobby.send({ content: `다음 대회: **${name}**\n일정: <t:${Math.floor(startsAt / 1000)}:f> ~ <t:${Math.floor(endsAt / 1000)}:f>`, components: [new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder().setCustomId(`join:${key}`).setLabel("참가할게").setEmoji("👋").setStyle(discord_js_1.ButtonStyle.Primary))] });
        contest = { ...contest, lobbyChannelId: lobby.id, lobbyMessageId: m.id };
        (0, store_1.putContest)(contest);
    }
    return contest;
}
...
client.once(discord_js_1.Events.ClientReady, async () => { for (const g of client.guilds.cache.values())
    if (!guildIds.length || guildIds.includes(g.id))
        await g.commands.set([command.toJSON()]); console.log(`DAWN online: ${client.user.tag}`); });
