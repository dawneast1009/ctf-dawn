...
exports.EVENT_KIND_ORDER = void 0;
exports.sha256 = sha256;
exports.compactText = compactText;
exports.stripHtml = stripHtml;
exports.normalizeGenres = normalizeGenres;
exports.participationMode = participationMode;
exports.parseDate = parseDate;
exports.extractDateTimes = extractDateTimes;
exports.periodAfterLabel = periodAfterLabel;
exports.looksLikeResultNews = looksLikeResultNews;
exports.isSecurityNews = isSecurityNews;
exports.isEventAnnouncement = isEventAnnouncement;
exports.classifyEvent = classifyEvent;
exports.parseEventNotice = parseEventNotice;
exports.normalizedEventTitle = normalizedEventTitle;
exports.eventDedupeKey = eventDedupeKey;
exports.eventContentHash = eventContentHash;
exports.eventRegion = eventRegion;
exports.bucketForEvent = bucketForEvent;
exports.eventDateLabel = eventDateLabel;
exports.shouldIncludeEvent = shouldIncludeEvent;
exports.interleaveEvents = interleaveEvents;
...
exports.EVENT_KIND_ORDER = ["ctf", "ai", "conference", "hackathon", "security", "news"];
function sha256(value, length = 20) {
    return (0, node_crypto_1.createHash)("sha256").update(value).digest("hex").slice(0, length);
}
...
function normalizeGenres(raw, text = "") {
    const values = [];
...
function participationMode(...values) {
...
function kstTimestamp(year, month, day, hour = 0, minute = 0) {
...
function extractDateTimes(text) {
    ...
    for (const match of text.matchAll(SHORT_DATE_RE)) {
        ...
        if (occupied.some(([a, b]) => !(end <= a || b <= start)))
            continue;
...
function periodAfterLabel(text, label) {
...
function classifyEvent(title, summary = "") {
    ...
    if (looksLikeResultNews(title))
        return "news";
    if (/해커톤|hackathon|아이디어톤|ideathon/i.test(text))
        return "hackathon";
    if (/컨퍼런스|conference|세미나|포럼\b|forum\b|secon/i.test(text))
        return "conference";
    if (/(?:인공지능|(?:^|\W)AI(?:\W|$)|머신러닝|딥러닝|데이터\s*분석|dacon|데이콘)/i.test(text) && /경진|대회|공모전|competition|challenge/i.test(text))
        return "ai";
    if (/\bCTF\b|capture\s+the\s+flag|해킹\s*방어\s*대회|사이버\s*(?:공격\s*)?방어\s*대회|ctftime|wacon/i.test(titleOnly))
        return "ctf";
    if (/정보보안|정보보호|사이버보안|화이트햇|해킹|보안/i.test(text) && /영재(?:원|교육원)?|교육생|수강생|아카데미|부트캠프|교육\s*(?:과정|프로그램)|캠프|멘토링|연수생|훈련생|공모전|진로\s*체험|특강|워크숍|장학생|동아리|모집/i.test(text))
        return "security";
...
function labeledValue(lines, labels) {
    for (const line of lines) {
        ...
        if (value !== line || line.length <= 120)
            return value;
    }
    return undefined;
}
function noticeName(lines) {
...
function parseEventNotice(content, source = "Direct") {
    const lines = content.split(/\r?\n/).map(cleanLine).filter(Boolean);
    if (!lines.length)
        throw new Error("공지 내용이 비어 있습니다.");
    ...
    if (middleSchoolOnly && !primaryType)
        throw new Error("중학생 전용 기타 공지는 수집 대상이 아닙니다.");
...
function eventContentHash(item) {
    const normalized = {
        title: item.title,
        link: item.link,
        source: item.source,
...
function eventRegion(item) {
    if (item.region === "kr" || item.region === "global")
        return item.region;
    if (["K-CTF", "한국코드페어", "CODEGATE 해커톤"].includes(item.source))
        return "kr";
    if (item.location) {
        if (KOREAN_LOCATION_RE.test(item.location))
            return "kr";
        if (FOREIGN_LOCATION_RE.test(item.location))
            return "global";
    }
...
function bucketForEvent(item, now = Date.now()) {
    if (item.kind === "news")
        return "latest";
    if (item.endsAt && item.endsAt < now)
        return "ended";
    if (!["conference", "security"].includes(item.kind ?? "") && /\bmain round\b|\bmain stage\b|\bfinals?\b|\bgrand final\b|본선|결승|데모 ?데이|demo ?day/i.test(item.title))
        return "final";
    const target = item.registrationDeadline ?? item.startsAt;
    if (!target)
        return "later";
    if (target <= now + 30 * DAY)
        return "within_1m";
    if (target <= now + 60 * DAY)
        return "within_2m";
    return "later";
}
...
function shouldIncludeEvent(item, options) {
    ...
    if (item.startsAt && item.endsAt && item.startsAt < now && now <= item.endsAt)
        return true;
    const target = item.registrationDeadline ?? item.startsAt;
    if (!target && item.manual)
        return true;
    return Boolean(target && target >= now - 12 * 60 * 60 * 1000 && target <= cutoff);
}
function interleaveEvents(events) {
    const dateKey = (event) => event.registrationDeadline ?? event.startsAt ?? event.endsAt ?? Number.MAX_SAFE_INTEGER;
    const queues = exports.EVENT_KIND_ORDER.map((kind) => events.filter((event) => (event.kind ?? "security") === kind).sort((a, b) => dateKey(a) - dateKey(b)));
    queues.push(events.filter((event) => !exports.EVENT_KIND_ORDER.includes((event.kind ?? "security"))).sort((a, b) => dateKey(a) - dateKey(b)));
    const ordered = [];
