...
exports.isEditorialUrl = isEditorialUrl;
exports.fetchNoticeText = fetchNoticeText;
exports.parseBoanNewsFeed = parseBoanNewsFeed;
exports.searchTitleMatchesKind = searchTitleMatchesKind;
exports.titleYearMatchesEvent = titleYearMatchesEvent;
exports.optionsFromEnv = optionsFromEnv;
exports.collectEventItems = collectEventItems;
...
async function withTimeout(work, timeoutMs = 20_000) {
...
async function settleInBatches(tasks, concurrency) {
    const results = [];
...
async function responseText(response) {
    ...
    const encoding = charset && iconv_lite_1.default.encodingExists(charset) ? charset : "utf-8";
...
function absoluteUrl(base, href) {
    ...
    try {
        ...
        if (url.hostname === "www.kisia.or.kr")
            url.hostname = "kisia.or.kr";
        return url.toString();
    }
    catch {
        return undefined;
...
function feedItems(xml) {
    const parser = new fast_xml_parser_1.XMLParser({ ignoreAttributes: false, removeNSPrefix: true, trimValues: true, cdataPropName: "__cdata" });
    let root;
    try {
    ...
    return rows.map((row) => {
        ...
        return {
            title: (0, core_1.stripHtml)(objectText(row?.title)),
            link,
            description: (0, core_1.stripHtml)(objectText(row?.description ?? row?.summary ?? row?.content)),
            publishedAt: (0, core_1.parseDate)(objectText(row?.pubDate ?? row?.published ?? row?.updated ?? row?.date)),
        };
    });
}
async function fetchNoticeText(url) {
    ...
    const links = main
        ...
        .get()
        .filter(Boolean);
    return (0, core_1.compactText)([url, title, main.text(), links.length ? `관련 링크\n${[...new Set(links)].join("\n")}` : ""].filter(Boolean).join("\n"));
}
...
async function fetchCtftime(lookaheadDays) {
    ...
    return [...rows.values()].flatMap((row) => {
        ...
        const summary = (0, core_1.stripHtml)(String(row.description ?? ""));
        const organizer = Array.isArray(row.organizers) ? row.organizers.map((item) => item?.name).filter(Boolean).join(", ") : undefined;
        const poster = row.logo ? absoluteUrl("https://ctftime.org/", String(row.logo)) : undefined;
        const startsAt = (0, core_1.parseDate)(row.start);
        const endsAt = (0, core_1.parseDate)(row.finish);
        return [{
                id: (0, core_1.sha256)(`CTFtime:${row.id}`), guildId: "", title: String(row.title), link, source: "CTFtime", kind: "ctf",
                summary, organizer, posterUrl: poster, startsAt, endsAt, publishedAt: startsAt ?? Date.now(),
                location: row.location ? String(row.location) : undefined,
                participationMode: (0, core_1.participationMode)(row.format, row.location, summary),
                genres: (0, core_1.normalizeGenres)(row.categories ?? row.tags, `${row.format ?? ""} ${summary} ${row.title}`),
            }];
    });
}
async function fetchJsonFeed(url) {
    ...
    return rows.flatMap((row) => {
        ...
        const startsAt = (0, core_1.parseDate)(row.start_at);
        const genres = (0, core_1.normalizeGenres)(row.genres ?? row.categories ?? row.tags, String(row.description ?? ""));
        return [{
                id: String(row.id ?? (0, core_1.sha256)(`${row.name}|${row.start_at ?? ""}|${link}`)), guildId: "", title: String(row.name), link,
                ...
                registrationDeadline: (0, core_1.parseDate)(row.registration_deadline), startsAt, endsAt: (0, core_1.parseDate)(row.end_at), publishedAt: startsAt ?? Date.now(),
                posterUrl: row.poster_url ?? row.image_url, genres, teamLimit: row.team_limit ? String(row.team_limit) : undefined,
                participationMode: row.participation_mode ? String(row.participation_mode) : (0, core_1.participationMode)(row.format, row.location, row.description),
                location: row.location ? String(row.location) : undefined,
            }];
    });
}
...
function canonicalBoanLink(link) {
    try {
...
function parseBoanNewsFeed(xml, now = Date.now()) {
    return feedItems(xml).slice(0, 10).flatMap((row) => {
        const link = canonicalBoanLink(row.link);
        if (!row.title || !link || !row.publishedAt || row.publishedAt < now - 30 * 86_400_000)
            return [];
        return [{
                id: (0, core_1.sha256)(link), guildId: "", title: row.title, link, source: "보안뉴스", kind: "news",
                summary: row.description.slice(0, 500) || undefined, publishedAt: row.publishedAt, startsAt: row.publishedAt,
            }];
    });
}
async function fetchBoanNews() {
    return parseBoanNewsFeed(await fetchText("https://www.boannews.com/media/news_rss.xml?mkind=1"));
}
function isKoreanCandidate(text, link) {
    try {
        if (/\.(?:kr|ac\.kr|go\.kr|or\.kr|re\.kr)$/.test(new URL(link).hostname))
            return true;
    }
    catch {
        return false;
...
function searchTitleMatchesKind(kind, title) {
    const eventSignal = /모집|접수|신청|참가|개최|안내|대회|경진|공모전|해커톤|컨퍼런스|세미나|포럼|캠프|교육생|아카데미|challenge|competition|conference|hackathon/i;
    if (!eventSignal.test(title))
        return false;
    if (/부임|신임\s*교수|환영|논문\s*채택|연구\s*성과|수상|입상|성료/i.test(title))
        return false;
    if (kind === "ctf")
        return /\bCTF\b|capture\s+the\s+flag|해킹\s*방어|사이버\s*(?:공격\s*)?방어|화이트햇\s*콘테스트|\bCCE\b|WACON/i.test(title);
    if (kind === "ai")
        return /(?:^|\W)AI(?:\W|$)|인공지능|머신러닝|데이터\s*(?:분석|경진)|DACON|데이콘/i.test(title) && /대회|경진|공모전|해커톤|challenge|competition/i.test(title);
    if (kind === "hackathon")
        return /해커톤|hackathon|아이디어톤|ideathon/i.test(title);
    if (kind === "conference")
        return /정보보안|정보보호|사이버보안|보안|CODEGATE|SECON/i.test(title) && /컨퍼런스|conference|세미나|포럼|forum|워크숍|전시회|개최/i.test(title);
    if (kind === "security")
        return /정보보안|정보보호|사이버보안|화이트햇|해킹|보안/i.test(title) && /모집|교육|캠프|아카데미|공모전|경진|멘토링|특강|워크숍/i.test(title);
    return (0, core_1.isSecurityNews)(title);
}
function titleYearMatchesEvent(title, event, now = Date.now()) {
...
async function feedItemToEvent(feed, item, lookaheadDays) {
    const combined = `${item.title}\n${item.description}\n${item.link}`;
    if (!item.title || !item.link || !SECURITY_RE.test(combined))
        return undefined;
    if (feed.kind === "news") {
        if (!(0, core_1.isSecurityNews)(item.title, item.description) || (item.publishedAt && item.publishedAt < Date.now() - 14 * 86_400_000))
            return undefined;
        return {
            id: (0, core_1.sha256)(item.link), guildId: "", title: item.title, link: item.link, source: feed.name, kind: "news",
            summary: item.description.slice(0, 500), publishedAt: item.publishedAt ?? Date.now(), startsAt: item.publishedAt ?? Date.now(),
        };
    }
    if (isEditorialUrl(item.link))
        return undefined;
    if (!searchTitleMatchesKind(feed.kind, item.title))
        return undefined;
    if (!(0, core_1.isEventAnnouncement)(item.title, item.description) || (0, core_1.looksLikeResultNews)(item.title, item.description))
        return undefined;
    let notice = combined;
    try {
        const loaded = await fetchNoticeText(item.link);
        if (loaded.length > notice.length)
            notice = loaded;
    }
    catch {
        // 검색 요약만으로도 파싱 가능한 경우 계속 진행한다.
    }
    if ((0, core_1.looksLikeResultNews)(notice))
        return undefined;
    let event;
    try {
        event = (0, core_1.parseEventNotice)(notice, feed.name);
    }
    catch {
        return undefined;
    }
    event.kind = feed.kind;
    event.id = (0, core_1.sha256)(item.link);
    event.link ||= item.link;
    event.registrationUrl ||= event.link;
    event.summary ||= item.description.slice(0, 500);
    event.manual = false;
    if (!titleYearMatchesEvent(item.title, event))
        return undefined;
    if (feed.kind === "hackathon" && !isKoreanCandidate(notice, item.link))
        return undefined;
    const target = event.registrationDeadline ?? event.startsAt;
    if (!target || target < Date.now() - 3 * 86_400_000 || target > Date.now() + lookaheadDays * 86_400_000)
        return undefined;
    return event;
}
async function fetchDiscoveryFeed(feed, lookaheadDays) {
    const items = feedItems(await fetchText(feed.url)).slice(0, feed.max);
    const converted = await settleInBatches(items.map((item) => () => feedItemToEvent(feed, item, lookaheadDays)), DETAIL_CONCURRENCY);
    return converted.flatMap((result) => result.status === "fulfilled" && result.value ? [result.value] : []);
}
async function fetchKctf() {
    ...
    $('a[href^="/contests/"]').each((_, node) => {
        ...
        if (/^\/contests\/[0-9a-f]+$/i.test(href))
            paths.add(href);
    });
    for (const match of html.matchAll(/location\.href=['"](\/contests\/[0-9a-f]+)['"]/gi))
        paths.add(match[1]);
    const details = await settleInBatches([...paths].map((path) => async () => {
        const url = new URL(path, base).toString();
        const detailHtml = await fetchText(url, 8_000);
        const detail = (0, cheerio_1.load)(detailHtml);
        ...
        const allDates = (0, core_1.extractDateTimes)(text);
        const registrationDates = (0, core_1.periodAfterLabel)(text, /신청\s*기간/i);
        ...
        const kind = /해커톤|hackathon/i.test(`${title} ${description ?? ""}`) ? "hackathon" : "ctf";
        return {
            id: (0, core_1.sha256)(`K-CTF:${path}`), guildId: "", title, link: official || url, source: "K-CTF", kind, summary: description,
            ...
            participationMode: (0, core_1.participationMode)(text), genres: (0, core_1.normalizeGenres)(undefined, `${title} ${description ?? ""}`), region: "kr",
        };
    }), DETAIL_CONCURRENCY);
...
async function fetchDacon() {
    ...
    const details = await settleInBatches([...candidates].map(([id, card]) => async () => {
        ...
        const detail = (0, cheerio_1.load)(detailHtml);
        const text = detail.root().text().replace(/\s+/g, " ");
        const period = (0, core_1.periodAfterLabel)(text, /대회\s*기간|예선\s*기간|대회\s*일정/i).slice(0, 2);
        let startsAt = period[0];
        let endsAt = period[1] ?? period[0];
        if (!startsAt) {
            const start = (0, core_1.extractDateTimes)(card.text)[0];
        ...
        return {
            id: (0, core_1.sha256)(`DACON:${id}`), guildId: "", title: card.title, link: url, source: "DACON", kind: "ai", summary,
            startsAt, endsAt, publishedAt: startsAt, organizer: "DACON", location: "온라인", participationMode: "온라인",
            posterUrl: card.poster ? absoluteUrl(base, card.poster) : undefined, genres: (0, core_1.normalizeGenres)(undefined, `AI ${card.title} ${summary ?? ""}`), region: "kr",
        };
    }), DETAIL_CONCURRENCY);
    return details.flatMap((result) => result.status === "fulfilled" && result.value ? [result.value] : []);
}
async function fetchGongil() {
    const base = "https://www.gongil.co.kr/";
    const $ = (0, cheerio_1.load)(await fetchText(base));
    const candidates = new Map();
    $('a[href^="/contests/"]').each((_, node) => {
        const anchor = $(node);
        const text = anchor.text().replace(/\s+/g, " ").trim();
        if (!text.startsWith("국내 ") || !/AI\/데이터|인공지능|AI\s|해커톤|정보보호|정보보안|사이버보안|해킹방어|CTF/i.test(text))
            return;
        const url = absoluteUrl(base, anchor.attr("href") ?? "");
        if (url)
            candidates.set(url, text);
    });
    const details = await settleInBatches([...candidates].slice(0, 12).map(([url, cardText]) => async () => {
        const detail = (0, cheerio_1.load)(await fetchText(url));
        const title = detail("title").first().text().split("|", 1)[0].trim();
        const text = detail.root().text().replace(/\s+/g, " ");
        const deadlineParts = /마감일\s*(20\d{2})-(\d{1,2})-(\d{1,2})/.exec(text);
        if (!title || !deadlineParts)
            return undefined;
        ...
        if (["dacon.io", "aichallenge4all.or.kr", "maicon.kr", "kcf.or.kr", "ctftime.org"].includes(host))
            return undefined;
        const deadline = Date.UTC(Number(deadlineParts[1]), Number(deadlineParts[2]) - 1, Number(deadlineParts[3]), 14, 59);
        const kind = /해커톤|hackathon/i.test(title) ? "hackathon" : /CTF|해킹방어/i.test(title) ? "ctf" : /AI|인공지능|데이터/i.test(title) ? "ai" : "security";
        return {
            id: (0, core_1.sha256)(url), guildId: "", title, link, source: "공일", kind, summary: cardText.slice(0, 500),
            registrationDeadline: deadline, publishedAt: deadline, participationMode: (0, core_1.participationMode)(text),
            genres: (0, core_1.normalizeGenres)(undefined, `${title} ${cardText}`), region: "kr",
        };
    }), DETAIL_CONCURRENCY);
    return details.flatMap((result) => result.status === "fulfilled" && result.value ? [result.value] : []);
}
async function fetchKiisc() {
    ...
    $('a[href*="/bbs/pe/article/"]').each((_, node) => {
        ...
        const startsAt = (0, core_1.parseDate)(rawDate) ?? (0, core_1.extractDateTimes)(rawDate)[0];
        const link = absoluteUrl(base, anchor.attr("href") ?? "");
        if (!title || !startsAt || !link)
            return;
        events.set(link, {
            id: (0, core_1.sha256)(link), guildId: "", title, link, source: "한국정보보호학회", kind: "conference",
    ...
    $(".tl-card3").each((_, node) => {
        ...
        const dates = (0, core_1.extractDateTimes)(rawDate);
        const link = absoluteUrl(base, card.find("a[href]").first().attr("href") ?? "");
        if (!title || !dates.length || !link)
            return;
        events.set(link, {
            id: (0, core_1.sha256)(link), guildId: "", title, link, source: "한국정보보호학회", kind: "conference",
...
function walkObjects(value) {
    if (Array.isArray(value))
        return value.flatMap(walkObjects);
    if (!value || typeof value !== "object")
        return [];
    const row = value;
    return [row, ...Object.values(row).flatMap(walkObjects)];
}
async function fetchJsonLd(name, url, kind) {
    const $ = (0, cheerio_1.load)(await fetchText(url));
    const events = [];
    $('script[type="application/ld+json"]').each((_, node) => {
        try {
            for (const row of walkObjects(JSON.parse($(node).text()))) {
                if (row["@type"] !== "Event" || !row.name)
                    continue;
                const link = String(row.url ?? url);
                const location = typeof row.location === "object" ? row.location?.name : row.location;
                const startsAt = (0, core_1.parseDate)(row.startDate);
                events.push({
                    id: (0, core_1.sha256)(`${name}|${row.name}|${row.startDate ?? ""}|${link}`), guildId: "", title: String(row.name), link, source: name, kind,
                    summary: row.description ? (0, core_1.stripHtml)(String(row.description)) : undefined, startsAt, endsAt: (0, core_1.parseDate)(row.endDate),
                    registrationDeadline: (0, core_1.parseDate)(row.registrationDeadline), publishedAt: startsAt ?? Date.now(),
                    ...
                    location: location ? String(location) : undefined, participationMode: (0, core_1.participationMode)(row.eventAttendanceMode, location, row.description),
                    genres: (0, core_1.normalizeGenres)(row.keywords, String(row.description ?? "")), region: ["conference", "hackathon"].includes(kind) ? "kr" : undefined,
                });
            }
        }
        catch {
...
async function fetchKoreanCodeFair() {
    const pages = [
    ...
    for (const [url, fallback, kind] of pages) {
        ...
        const registration = (0, core_1.periodAfterLabel)(text, /접수\s*기간|접수기간|모집/i).slice(0, 2);
        if (!registration.length)
            continue;
        const edition = /제\s*(\d+)\s*회\s*한국코드페어/.exec(text)?.[1];
        const title = `${edition ? `제${edition}회 ` : ""}한국코드페어 ${kind === "hackathon" ? "해커톤" : "SW공모전"}`;
        results.push({
            id: (0, core_1.sha256)(`${fallback}|${new Date(registration[0]).getUTCFullYear()}|${url}`), guildId: "", title, link: url, source: "한국코드페어", kind,
            ...
            teamLimit: "개인 또는 3인 이하 팀", participationMode: (0, core_1.participationMode)(text), genres: (0, core_1.normalizeGenres)(undefined, text), region: "kr",
        });
    }
    return results;
}
async function fetchOfficialBoard(board) {
    const $ = (0, cheerio_1.load)(await fetchText(board.url));
    const candidates = new Map();
    $("a[href]").each((_, node) => {
        ...
        if (title && link && board.titlePattern.test(title) && board.linkPattern.test(link))
            candidates.set(link, title);
    });
    const details = await settleInBatches([...candidates].map(([url, title]) => async () => {
        ...
        let event;
        try {
            event = (0, core_1.parseEventNotice)(`${title}\n${notice}`, board.name);
        }
        catch {
            event = { id: (0, core_1.sha256)(url), guildId: "", title, link: url, source: board.name, kind: board.kind, publishedAt: Date.now() };
        }
        event.id = (0, core_1.sha256)(url);
        ...
        event.manual = false;
        if (board.kind === "news") {
            event.startsAt = (0, core_1.extractDateTimes)(notice)[0];
        ...
        else if (!event.registrationDeadline && !event.startsAt)
            return undefined;
        return event;
    }), DETAIL_CONCURRENCY);
    return details.flatMap((result) => result.status === "fulfilled" && result.value ? [result.value] : []);
...
async function fetchGenericPage(url) {
    ...
    $("a[href]").slice(0, 400).each((_, node) => {
        ...
        if (link && title.length >= 4 && title.length <= 180 && SECURITY_RE.test(title) && (0, core_1.isEventAnnouncement)(title))
            candidates.set(link, title);
    });
    const events = await settleInBatches([...candidates].slice(0, 12).map(([link, title]) => async () => {
        const notice = await fetchNoticeText(link);
        if ((0, core_1.looksLikeResultNews)(title, notice))
            return undefined;
        const event = (0, core_1.parseEventNotice)(`${title}\n${notice}`, source);
        event.id = (0, core_1.sha256)(link);
        event.link = link;
...
async function naverSearch(options, query, target) {
    ...
    const payload = await fetchJson(url, { headers: { "X-Naver-Client-Id": options.naverClientId, "X-Naver-Client-Secret": options.naverClientSecret } });
    return list(payload?.items).flatMap((row) => {
        const title = (0, core_1.stripHtml)(String(row?.title ?? ""));
        const link = String(row?.originallink || row?.link || "");
        return title && link ? [{ provider: `NaverSearch-${target}`, title, url: link, snippet: (0, core_1.stripHtml)(String(row?.description ?? "")), publishedAt: (0, core_1.parseDate)(row?.pubDate) }] : [];
    });
}
async function googleSearch(options, query) {
    ...
    const payload = await fetchJson(url);
    return list(payload?.items).flatMap((row) => row?.title && row?.link ? [{ provider: "GoogleCustomSearch", title: (0, core_1.stripHtml)(String(row.title)), url: String(row.link), snippet: (0, core_1.stripHtml)(String(row.snippet ?? "")) }] : []);
}
async function bingSearch(options, query) {
    ...
    const payload = await fetchJson(url, { headers: { "Ocp-Apim-Subscription-Key": options.bingApiKey } });
    return list(payload?.webPages?.value).flatMap((row) => row?.name && row?.url ? [{ provider: "BingWebSearch", title: (0, core_1.stripHtml)(String(row.name)), url: String(row.url), snippet: (0, core_1.stripHtml)(String(row.snippet ?? "")), publishedAt: (0, core_1.parseDate)(row.dateLastCrawled) }] : []);
}
async function searchResultToEvent(result, query, lookaheadDays) {
    const combined = `${result.title}\n${result.snippet}\n${result.url}`;
    if (!result.title || !result.url || BLOCKED_URL_RE.test(result.url) || !SECURITY_RE.test(combined))
        return undefined;
    if (query.kind === "news") {
        if (!(0, core_1.isSecurityNews)(result.title, result.snippet) || (result.publishedAt && result.publishedAt < Date.now() - 30 * 86_400_000))
            return undefined;
        return { id: (0, core_1.sha256)(`${result.provider}|${result.url}`), guildId: "", title: result.title, link: result.url, source: query.name, kind: "news", summary: result.snippet.slice(0, 500), publishedAt: result.publishedAt ?? Date.now(), startsAt: result.publishedAt ?? Date.now() };
    }
    if (isEditorialUrl(result.url))
        return undefined;
    if (!searchTitleMatchesKind(query.kind, result.title))
        return undefined;
    if (!(0, core_1.isEventAnnouncement)(result.title, result.snippet) || (0, core_1.looksLikeResultNews)(result.title, result.snippet))
        return undefined;
    let notice = combined;
    try {
    ...
    let event;
    try {
        event = (0, core_1.parseEventNotice)(notice, query.name);
    }
    catch {
        return undefined;
    }
    event.id = (0, core_1.sha256)(`${result.provider}|${result.url}`);
    event.kind = query.kind;
    event.link ||= result.url;
    event.registrationUrl ||= event.link;
    event.summary ||= result.snippet.slice(0, 500);
    event.manual = false;
    if (!titleYearMatchesEvent(result.title, event))
        return undefined;
    if (query.kind === "hackathon" && !isKoreanCandidate(notice, result.url))
        return undefined;
    const target = event.registrationDeadline ?? event.startsAt;
    if (query.requireDate && !target)
        return undefined;
    if (target && (target < Date.now() - 3 * 86_400_000 || target > Date.now() + lookaheadDays * 86_400_000))
        return undefined;
    return event;
}
async function fetchSearchApi(options) {
    const tasks = SEARCH_QUERIES.flatMap((query) => {
        const providers = [
            () => naverSearch(options, query, "webkr").then((rows) => [query, rows]),
            () => googleSearch(options, query).then((rows) => [query, rows]),
            () => bingSearch(options, query).then((rows) => [query, rows]),
        ];
        if (query.kind === "news")
            providers.push(() => naverSearch(options, query, "news").then((rows) => [query, rows]));
        return providers;
    });
    ...
    for (const result of searched) {
        ...
        for (const row of rows)
            conversions.push(() => searchResultToEvent(row, query, options.lookaheadDays));
    }
...
function optionsFromEnv() {
    ...
    const bool = (name, fallback) => process.env[name] == null ? fallback : /^(?:1|true|yes|on)$/i.test(process.env[name] ?? "");
    return {
...
async function collectEventItems(options = optionsFromEnv()) {
    ...
    if (options.enableKctf)
        sources.push({ name: "K-CTF", fetch: fetchKctf });
    if (options.enableAutoDiscovery) {
        const feeds = options.discoveryFeedUrls.length
            ? options.discoveryFeedUrls.map((url, index) => ({ name: `사용자자동탐색-${index + 1}`, url, kind: "security", max: 8 }))
    ...
    const results = await settleInBatches(sources.map((source) => source.fetch), options.sourceConcurrency);
    const items = [];
    const errors = [];
    for (let index = 0; index < results.length; index++) {
        ...
        if (result.status === "fulfilled")
            ...
            errors.push(`${sources[index].name}: ${result.reason instanceof Error ? result.reason.message : String(result.reason)}`);
    }
    return { items, errors };
}
