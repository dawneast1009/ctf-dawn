...
import { detectPlatform, parseHspaceChallenges } from "../src/ctf/platforms";
...
test("HSPACE FORGE 문제 목록을 DAWN 형식으로 변환한다", async () => {
  assert.equal(await detectPlatform("https://forge.hspace.io/competitions/6905f6cba3e85ec8046e4ff4"), "hspace");
  assert.deepEqual(parseHspaceChallenges({ data: [{ _id: "abc", title: "Travel the World", category: "Web" }] }), [
    { externalId: "abc", name: "Travel the World", category: "Web" },
  ]);
});
